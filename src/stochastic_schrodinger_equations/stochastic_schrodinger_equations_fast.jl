...
function initialize_prob(
    ...
    p = MutableNamedTuple(
        ...
        add_force_from_hamiltonian=add_force_from_hamiltonian,
        sats=sats
    )
...
function ψ_fast!(du, u, p, t)
    ...
    p.add_force_from_hamiltonian(p.F, p.ψ, p.sim_params, p.r)
...
@inline function update_du_ballistic!(du, u, dψ, ψ, n_states, n_g, r_idx, F, v_idx, F_idx, m)
    ...
    @inbounds @fastmath for k ∈ 1:3
        du[v_idx+k] = 0.
        # du[r_idx+k] = 0. # no significant motion in tweezer trap.
    end
end
